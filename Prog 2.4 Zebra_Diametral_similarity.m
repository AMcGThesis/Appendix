...
SigP = [1,1.6/1.51,2/1.51,3/1.51]; %Modified scf to determine stress up wand for a given scf, relative to pole base.
hold on
   plot(l(:)    /lo,   SigP(1)*Sig3*Sig(1,:)   ,'linewidth', 1.5,'color', [0, 0.4470, 0.7410] )
   plot(l(2:end)/lo,   SigP(1)*Sig3*Sig(2,2:end),'linewidth', 1.5,'color', [0.8500, 0.3250, 0.0980] )

   plot(l(:)    /lo,   SigP(2)*Sig3*Sig(1,:)    ,'--','linewidth', 1.5,'color', [0, 0.4470, 0.7410])
   plot(l(2:end)/lo,   SigP(2)*Sig3*Sig(2,2:end),'--','linewidth', 1.5,'color', [0.8500, 0.3250, 0.0980])
      set(gca,'FontSize',20)
      grid on
xlabel('Scale Ratio [l/l_o]')
ylabel('Stress [MPa]')
legend('Base Stress, 2.5mm wall thickness','Base Stress, 5mm wall thickness','K=1.6, 2.5mm wall thickness','K=1.6, 5mm wall thickness')
